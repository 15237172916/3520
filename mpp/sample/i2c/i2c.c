...
static HI_RET atoul(IN CHAR *str,OUT U32 * pulValue);
static HI_RET atoulx(IN CHAR *str,OUT U32 * pulValue);
...
static HI_RET StrToNumber(IN CHAR *str , OUT U32 * pulValue)
{
...
BOOL i2c_write_byte( BYTE address,BYTE offset,BYTE byteno,BYTE *p_data,BYTE device )
{
	...
	for (i=0; i<byteno; i++)
	{
		...
		sprintf(write_cmd[2], "0x%x", address); //device_addr
		sprintf(write_cmd[3], "0x%x", offset); //reg_addr
		sprintf(write_cmd[4], "0x%x", *p_data); //value
		sprintf(write_cmd[5], "%d", 1); //reg_width
		sprintf(write_cmd[6], "%d", 1); //data_width

		ret = i2c_write( 7, write_cmd);
		if (ret < 0)
		{
			return 0;
		}
